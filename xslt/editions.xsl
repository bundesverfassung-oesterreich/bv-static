...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <body class="page" lang="de">
                <div id="text_quality_disclaimer" class="offcanvas offcanvas-start show"
                    tabindex="-1" aria-labelledby="tqd_label" data-bs-scroll="false"
                                    ...
                                    <div id="viewer">
                                        ...
                                        <div class="image_rights">
                                            <div class="row">
                                                ...
                                                <button class="osd_nav_element"
                                                  id="osd_zoom_in_button">+</button>
                                            </div>
                                            <p>Das Original befindet sich im Eigentum des
                                                Österreichischen Staatsarchivs unter der <span
                                                  style="font-weight: bold;">ÖStA-Signatur „
                                                  <xsl:value-of
                                                  select='//tei:msDesc/tei:msIdentifier/tei:idno[@type = "archive"]/text()[1]/normalize-space()'
    ...
    <xsl:template
        match="text()[preceding-sibling::*[1][local-name() = 'lb' and @break = 'no' and not(preceding-sibling::node()[self::text()])]]">
        <xsl:value-of select="normalize-space(replace(., '^\s*(.+?)$', '$1'))"/>
    </xsl:template>
                ...
                <xsl:choose>
                    <xsl:when test="./tei:label">
                        ...
                        <xsl:variable name="label_text">
                            <xsl:choose>
                                <xsl:when
                                    test="$label_element/following-sibling::text()[starts-with(., ' ')]">
                                    <xsl:value-of select="$label_element/normalize-space()"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of
                                        select="concat($label_element/normalize-space(), ' ')"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
